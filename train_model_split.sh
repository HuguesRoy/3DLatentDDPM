...
CONFIG_NAME=train_vp_${SLURM_ARRAY_TASK_ID}

# Move to the project root if needed
cd /gpfswork/rech/krk/uqo89gi/projects/3DLatentDDPM/src

# Run the Hydra training script
echo HYDRA_FULL_ERROR=1 python train.py --config-path $CONFIG_PATH --config-name $CONFIG_NAME

HYDRA_FULL_ERROR=1 python train.py --config-path $CONFIG_PATH --config-name $CONFIG_NAME
